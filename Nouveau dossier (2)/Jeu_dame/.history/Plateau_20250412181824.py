...
class Plateau:
    ...
    def obtenir_capture_multiple(self, pion, mouvement):
        ...
        """
        captures = []
        self.deplacer_pion(mouvement)  # Effectuer la capture initiale
        if self.peut_encore_manger(pion):
            mouvements_suivants = self.cases_fin_manger(pion)
            for prochain_mouvement in mouvements_suivants:
                captures.extend(self.obtenir_capture_multiple(pion, Mouvement(pion.position, prochain_mouvement)))
        else:
