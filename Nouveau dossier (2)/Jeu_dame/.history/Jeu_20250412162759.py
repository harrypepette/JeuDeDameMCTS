        ...
        if self.selection is None:
            ...
            if Regles.mouvement_valide(pion, mouvement, self.plateau):
                ...
                if est_terminee:
                    ...
                    # Si c'est au tour des noirs, lancer la simulation
                    if self.joueurs[self.joueur_actuel].couleur == "noir":
                        self.lancer_simulation()


    def lancer_simulation(self):
        """
        Lance une simulation pour déterminer le meilleur mouvement pour les noirs.
        """
        print("Lancement de la simulation pour les noirs...")
        simulation = Simulation(self.plateau, self.joueurs[self.joueur_actuel])
        meilleur_mouvement = simulation.simuler_meilleur_mouvement()
        print(f"Le meilleur mouvement pour les noirs est : {meilleur_mouvement}")

        # Appliquer le meilleur mouvement pour les noirs
        mouvement = Mouvement(meilleur_mouvement[0], meilleur_mouvement[1])
        self.plateau.deplacer_pion(mouvement)
        self.joueur_actuel = 1 - self.joueur_actuel  # Repasser au joueur blanc