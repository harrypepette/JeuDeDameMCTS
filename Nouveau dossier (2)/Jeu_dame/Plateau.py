...
class Plateau:
    ...
    def get_case(self, x, y):
        """
        Récupère une case spécifique du plateau.
        :param x: Coordonnée x de la case.
        :param y: Coordonnée y de la case.
        :return: L'objet Case aux coordonnées spécifiées, ou None si les coordonnées sont invalides.
        """
        if 0 <= x < 8 and 0 <= y < 8:
            return self.cases[y][x]
        return None
    ...
    def obtenir_capture_multiple(self, pion, mouvement):
        """
        Retourne une liste des mouvements de capture multiples possibles.
        :param pion: Le pion ou la dame pour lequel on veut vérifier les captures multiples.
        :param mouvement: Le mouvement initial de capture.
        :return: Une liste de séquences de captures multiples.
        """
        captures = []
    
        # Créer une copie du plateau et du pion
        plateau_copie = self.copie_sans_surface()
        pion_copie = plateau_copie.get_pion(*pion.position)
    
        # Effectuer la capture sur la copie
        plateau_copie.deplacer_pion(mouvement)
    
        # Mettre à jour la position du pion après déplacement
        nouvelle_position = mouvement.arrivee
        pion_copie = plateau_copie.get_pion(*nouvelle_position)
    
        if pion_copie and plateau_copie.peut_encore_manger(pion_copie):
            mouvements_suivants = plateau_copie.cases_fin_manger(pion_copie)
            for prochain_mouvement in mouvements_suivants:
                sous_captures = plateau_copie.obtenir_capture_multiple(
                    pion_copie, 
                    Mouvement(pion_copie.position, prochain_mouvement)
                )
                for sous_capture in sous_captures:
                    captures.append([mouvement] + sous_capture)
        else:
            captures.append([mouvement])  # Ajouter la capture actuelle comme une séquence complète
