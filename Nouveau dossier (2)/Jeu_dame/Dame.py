...
class Dame(Pion):
    ...
    def check_manger(self, plateau):
        ...
        """
         # Importer Regles pour valider les mouvements

        cases_a_manger = []
        directions = [(-1, -1), (1, -1), (-1, 1), (1, 1)]  # Diagonales
        for dx, dy in directions:
            x, y = self.position
            positions_sur_chemin = []  # Stocker toutes les positions sur la diagonale
            pion_adverse_position = None  # Position du pion adverse trouvé

            # Explorer toute la diagonale
            while True:
                x += dx
                y += dy
                if not (0 <= x < 8 and 0 <= y < 8):
                    break
                positions_sur_chemin.append((x, y))
                pion = plateau.get_pion(x, y)
                if pion:
                    if pion.couleur != self.couleur:
                        pion_adverse_position = (x, y)  # Trouver un pion adverse
                    else:
                        break  # Si un pion de la même couleur est trouvé, arrêter
                    break  # Arrêter après avoir trouvé un pion (adverse ou non)

            # Si un pion adverse a été trouvé, vérifier les cases derrière
            if pion_adverse_position:
                x, y = pion_adverse_position
                while True:
                    x += dx
                    y += dy
                    if not (0 <= x < 8 and 0 <= y < 8):
                        break
                    if plateau.get_pion(x, y) is None:
                        # Vérifier si le mouvement est valide avec Regles.mouvement_valide
                        mouvement = Mouvement(self.position, (x, y))
                        if Regles.mouvement_valide(self, mouvement, plateau):
                            cases_a_manger.append((x, y))
                    else:
                        break  # Arrêter si une autre pièce est trouvée
