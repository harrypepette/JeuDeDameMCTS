...
class Plateau:
    ...
    def afficher(self, screen, mouvements_disponibles=None, cases_fin_manger=None):
        """
        Affiche le plateau.
        :param screen: L'écran Pygame.
        :param mouvements_disponibles: Liste des positions disponibles pour un déplacement.
        ...
        for x in range(8):
            for y in range(8):
                ...
                if mouvements_disponibles and (x, y) in mouvements_disponibles:
    ...
    def deplacer_pion(self, mouvement):
        ...
        if isinstance(pion, Dame):
            # Pour une dame, elle doit finir sur la case derrière le pion capturé
            self.cases[mouvement.arrivee[1]][mouvement.arrivee[0]] = pion
        else:
            # Pour un pion normal, il termine sur la case d'arrivée
            self.cases[mouvement.arrivee[1]][mouvement.arrivee[0]] = pion
    ...
    def mouvements_disponibles(self, pion, forcer_manger=False):
        """
        ...
        """
        if isinstance(pion, Dame):
            return pion.mouvements_disponibles(self, forcer_manger=forcer_manger)  # Utiliser la logique spécifique aux dames
